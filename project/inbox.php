<html>
    ...
    <script src="button.js" ></script></head>
    <body>
    <?php
...
<?php   
    ...
    if($_SESSION['username']){
        ...
        while($record=mysqli_fetch_assoc($result)){
            ...
            if(in_array($record['from_user'],$friends)&& isspam($record['message'])=='ham' && $record['deletemail']==0){
                ...
                if($record['notification']=="unread"){
                    echo'<button type="submit"  style="background:#828079"name="id" value='.$record["id"].'>From: '.$record["from_user"].' Subject: '.$record["subject"].' <span>'.$time.'</span></button><br>';
                }
                else{
                    echo'<button type="submit"  style="background:silver"name="id" value='.$record["id"].'>From: '.$record["from_user"].' Subject: '.$record["subject"].' <span>'.$time.'</span></button><br>';
                }
        ...
        while($record=mysqli_fetch_assoc($result)){
            ...
            if(!in_array($record['from_user'],$friends)&& isspam($record['message'])=='ham' && $record['deletemail']!=1){
                if($date<$record['mail_expire'] || $record['mail_expire']=="0000-00-00 00:00:00" || $record['mail_expire']=="1970-01-01 01:00:00")
                if($record['notification']=="unread"){
                    echo'<button type="submit"  style="background:#828079"name="id" value='.$record["id"].'>From: '.$record["from_user"].' Subject: '.$record["subject"].' <span>'.$time.'</span></button><br>';
                }
    ...
    ?>
        <?php
        $user=$_SESSION['username'];
        $query="SELECT * FROM attach WHERE to_user=? ORDER BY sent_time DESC";
        $stmt=mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt,"s",$user);
        mysqli_stmt_execute($stmt);
        $result=mysqli_stmt_get_result($stmt);
        ...
        while($record=mysqli_fetch_assoc($result)){
            $time=strtotime($record["sent_time"]);
            $time=date("d F Y H:i", $time);
            echo'<a href="uploads/doc_'.$record['name'].'" download><button>From: '.$record['from_user'].' Subject: '.$record['subject'].'<span>'.$time.'</span></button></a>';
            echo'<br><br>';
